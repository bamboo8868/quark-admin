  ...
  <div class="main">
    <el-form ref="formRef" :inline="true" :model="form" class="search-form bg-bg_color w-full pl-8 pt-3 overflow-auto">
      <el-form-item label="标签名称" prop="name">
        <el-input v-model="form.name" placeholder="搜索标签名称" clearable class="w-45!" />
      </el-form-item>
      <el-form-item>
    ...
    <PureTableBar title="游戏标签" :columns="columns" @refresh="onSearch">
      <template #buttons>
      ...
      <template v-slot="{ size, dynamicColumns }">
        <pure-table align-whole="center" showOverflowTooltip table-layout="auto" :loading="loading" :size="size"
          ...
          <template #operation="{ row, size }">
            ...
            <el-popconfirm :title="`是否确认删除标签 ${row.name}`" @confirm="handleDelete(row)">
              <template #reference>
